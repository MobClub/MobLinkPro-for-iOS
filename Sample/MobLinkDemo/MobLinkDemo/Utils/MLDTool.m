...
//线上
static NSString *const baseShareUrl = @"http://f.moblink.mob.com/demoPro/";
//static NSString *const baseShareUrl = @"http://10.18.97.58:2122/demoPro/";
@interface MLDTool()
                 ...
                 onView:(UIView *)onView
{
    ...
    else
    {
        if (path)
        {
            ...
            urlStr = [NSString stringWithFormat:@"%@%@/index.html",baseShareUrl, path];
            if (mobid)
            {
                urlStr = [NSString stringWithFormat:@"%@%@/index.html?mobid=%@",baseShareUrl, path, mobid];
            }
        }
        else
        {
            urlStr = [NSString stringWithFormat:@"%@/index.html?mobid=%@",baseShareUrl, mobid];
        }
    }
